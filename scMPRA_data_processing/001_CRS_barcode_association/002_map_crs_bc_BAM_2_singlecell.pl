...
#!!! Changeed for single cell: allow 3 mismatches alogn the full Guide-BC combo 
#!!! Changed for single cell: one input extra (guide BC FASTQ)

my ($bc, $guide ,$bam, $outfile) = @ARGV;
...
open(BC, "zcat $bc|");
open(GUIDE, "zcat $guide|");
open(BAM, "samtools view $bam| "); my $bamline;


#!!! Changed for single cell: go throguh both guide BC and GFP BC file
#!!! In the end concatenate the two barcodes
sub get_next_barcode {
  ...
  for (my $i=1;$i<=4;$i++) {
    ...
    return undef unless ($guideline = <GUIDE>);
    chomp $bcline;
    chomp $guideline;
    $readname = $bcline if $i == 1;
    $barcode = $bcline if $i == 2;
    $guide = $guideline if $i == 2;
  }
...
sub cigar_to_score {
  ...
  while ($bamline = <BAM>) {
    ...
    if ($bamsplit[0] ne $readname) { #if the bam file is ahead
      ...
      #!!! changed for single cell: needs one reverse mapping or one forward mapping
      #!!! I think the mappings should all be reverse but am not sure right now
      my @considered = grep {$maps_count_fwd{$_} == 1 or $maps_count_rev{$_} == 1} keys %maps_score;
...
#SortBC
  ...
  while (my $this_bc = shift @BARCODES) {
    ...
    my @MATCHES;
    if (@BARCODES) {
      @MATCHES = amatch($this_bc,  ["S3","I0","D0"], @BARCODES); #find approx. matches with at least one substitution in all barcodes with more reads
    }
